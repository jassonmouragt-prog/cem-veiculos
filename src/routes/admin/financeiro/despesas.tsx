      ...
      <main className="flex-1 p-4 sm:p-6 lg:p-8 max-w-7xl w-full mx-auto space-y-6">
        ...
        <Tabs defaultValue="veiculos" className="space-y-4">
          <TabsList className="bg-[#121212] border border-white/10 rounded-lg p-1">
            ...
            <TabsTrigger value="todas" className="text-xs">Todas as Despesas</TabsTrigger>
          </TabsList>

          <TabsContent value="veiculos">
            <div className="space-y-4">
              {vehicles.filter(v => getExpensesFromStorage().some(e => e.vehicleId === v.id)).map((vehicle) => {
                  ...
                  <Card key={vehicle.id} className="bg-[#121212] border-white/5 rounded-xl">
                    <CardHeader className="pb-3 border-b border-white/5">
                      <div className="flex flex-row items-center justify-between">
                        <div>
                          <CardTitle className="text-sm font-bold text-white">{vehicle.name}</CardTitle>
                    ...
                    <CardContent className="p-3">
                      <div className="space-y-2">
                        {vehicleExpenses.slice(0, 5).map((e) => (
                          <div key={e.id} className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2 p-2 bg-black/30 rounded-lg border border-white/5">
                            <div className="flex-1 min-w-0">
                              <p className="font-medium text-white text-sm">{e.description}</p>
                              <p className="text-xs text-gray-400">{e.category} • {formatDate(e.expenseDate)}</p>
                            </div>
                            <div className="flex items-center gap-2 shrink-0">
                              <span className="font-bold text-white">{formatCurrency(e.amount)}</span>
                              <Badge variant="outline" className={`text-[10px] ${
                                e.paymentStatus === "pago" ? "bg-emerald-500/15 text-emerald-400" :
                                e.paymentStatus === "atrasado" ? "bg-red-500/15 text-red-400" :
                        ...
                        {vehicleExpenses.length > 5 && (
                          <p className="text-xs text-gray-400 text-center py-2">+{vehicleExpenses.length - 5} despesas</p>
                        )}
                      </div>
                    </CardContent>
                  </Card>
              ...
              <CardContent>
                <div className="space-y-3">
                  {expenses.slice(0, 100).map((e) => {
                    ...
                    return (
                      <div key={e.id} className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 p-3 bg-black/30 rounded-lg border border-white/5">
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2 text-sm flex-wrap">
                            <span className="font-medium text-white">{e.description}</span>
                            <Badge variant="outline" className="text-[10px] bg-[#E8231F]/15 text-[#E8231F]">{e.category}</Badge>
                            <Badge variant="outline" className={`text-[10px] ${
                              e.paymentStatus === "pago" ? "bg-emerald-500/15 text-emerald-400" :
                              e.paymentStatus === "atrasado" ? "bg-red-500/15 text-red-400" :
                              "bg-amber-500/15 text-amber-400"
