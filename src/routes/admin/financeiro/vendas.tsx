...
function VendasPage() {
  ...
  const [vehicleToFinalize, setVehicleToFinalize] = useState<{ id: string; name: string; price: number; acquisitionCost?: number } | null>(null);
          ...
          {sales.length === 0 ? (
            <Card className="bg-[#121212] border-white/5 rounded-2xl p-12 text-center">
              ...
              <p className="text-xs text-gray-400 max-w-sm mx-auto">
                Finalize a primeira venda clicando em "Finalizar Venda" ou marcando um veículo como vendido.
              </p>
            </Card>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
              {sales.map((sale) => {
                ...
                return (
                  <Card key={sale.id} className="bg-[#121212] border-white/5 overflow-hidden flex flex-col rounded-2xl shadow-md hover:border-white/15 transition-all">
                    <CardHeader className="flex flex-row items-center justify-between pb-3 border-b border-white/5 px-4 pt-4">
                      <CardTitle className="text-sm font-bold text-white truncate">{sale.vehicle?.name || `Veículo ${sale.vehicleId.slice(0, 8)}`}</CardTitle>
                      <DropdownMenu>
                    ...
                    <CardContent className="p-4 flex-1 flex flex-col space-y-3">
                      <div className="grid grid-cols-2 gap-3 text-xs">
                        ...
                        <div>
                          ...
                          <p className="font-medium text-white">{formatCurrency(sale.commissionValue)}</p>
                        </div>
                        <div>
                          <span className="text-gray-400">Forma Pagamento</span>
                          <p className="font-medium text-white capitalize">{sale.paymentMethod.replace("_", " ")}</p>
                        </div>
                        <div>
                      ...
                      <div className="pt-2 border-t border-white/5 space-y-2">
                        <div className="flex justify-between text-sm">
                          ...
                          <span className="font-medium text-white">{formatCurrency(sale.announcedPrice)}</span>
                        </div>
                        <div className="flex justify-between text-sm">
                          <span className="text-gray-400">Final</span>
                          <span className="font-bold text-white">{formatCurrency(sale.finalPrice)}</span>
                        </div>
                        {sale.discountAmount > 0 && (
                          <div className="flex justify-between text-sm text-red-400">
                            <span>Desconto</span>
                            <span>{formatCurrency(sale.discountAmount)} ({sale.discountPercent.toFixed(1)}%)</span>
                          </div>
                        )}
                      ...
                      <div className="flex items-center gap-2 pt-2 border-t border-white/5">
                        <Badge variant="outline" className={`text-[10px] font-semibold px-2 py-0.5 rounded-full ${statusBadge}`}>
                          {sale.status}
                        </Badge>
                        <Button
                          variant="ghost"
