      ...
      <main className="flex-1 p-4 sm:p-6 lg:p-8 max-w-7xl w-full mx-auto space-y-6">
          ...
          <Card className="bg-[#121212] border-white/5 rounded-2xl">
            <CardContent className="p-5 space-y-2">
              ...
              <p className="text-2xl font-bold text-emerald-400">
                {formatCurrency(commissionTransactions.filter(t => t.status === "pago").reduce((s, t) => s + t.amount, 0))}
              </p>
            </CardContent>
          </Card>
          <Card className="bg-[#121212] border-white/5 rounded-2xl">
            <CardContent className="p-5 space-y-2">
              ...
              <p className="text-2xl font-bold text-amber-400">
                {formatCurrency(commissionTransactions.filter(t => t.status === "pendente").reduce((s, t) => s + t.amount, 0))}
              </p>
            </CardContent>
          </Card>
          <Card className="bg-[#121212] border-white/5 rounded-2xl">
        ...
        <Tabs defaultValue="resumo" className="space-y-4">
          <TabsList className="bg-[#121212] border border-white/10 rounded-lg p-1">
            <TabsTrigger value="resumo" className="text-xs">Resumo por Vendedor</TabsTrigger>
                  ...
                  <CardContent className="p-5">
                    <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
                      <div className="md:col-span-2">
                        ...
                        <p className="text-xs text-gray-400">{r.salesCount} vendas • Ticket médio: {formatCurrency(r.averageTicket)}</p>
                      </div>
          ...
          <TabsContent value="lancamentos">
            <Card className="bg-[#121212] border-white/5 rounded-xl">
              <CardHeader className="pb-3 border-b border-white/5">
                <CardTitle className="text-base font-bold text-white">Lançamentos de Comissão</CardTitle>
              </CardHeader>
              <CardContent>
                <div className="space-y-3">
                  {commissionTransactions.slice(0, 50).map((t) => (
                    <div key={t.id} className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 p-3 bg-black/30 rounded-lg border border-white/5">
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 text-sm">
                          <span className="font-medium text-white">{t.description}</span>
                          <Badge variant="outline" className={`text-[10px] ${
                            t.status === "pago" ? "bg-emerald-500/15 text-emerald-400" :
                            t.status === "atrasado" ? "bg-red-500/15 text-red-400" :
                            "bg-amber-500/15 text-amber-400"
                          }`}>
                            {t.status}
                          </Badge>
                        </div>
                        <p className="text-xs text-gray-400 mt-1">{formatDate(t.transactionDate)} • {t.seller?.name || "Vendedor"}</p>
                      </div>
                      <div className="flex items-center gap-4 shrink-0">
                        <span className="font-bold text-white whitespace-nowrap">{formatCurrency(t.amount)}</span>
                        {t.status === "pendente" && (
                          <Button size="sm" variant="outline" className="h-8 text-xs border-emerald-500/50 text-emerald-400">
                            Marcar Pago
                          </Button>
                        )}
                      </div>
