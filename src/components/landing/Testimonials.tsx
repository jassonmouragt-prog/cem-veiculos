...
export function Testimonials() {
  const reviews = [
    {
      name: "Rafael Santos",
      text: "Excelente atendimento do início ao fim. Transparência, veículo impecável e total respeito com o cliente. Recomendo demais!",
      photo: "https://i.pravatar.cc/150?u=rafael",
    },
    {
      name: "Juliana Oliveira",
      text: "Comprei meu Corolla aqui e foi a melhor experiência. Carro de procedência comprovada e equipe atenciosa e nota 10!",
      photo: "https://i.pravatar.cc/150?u=juliana",
    },
    {
      name: "Marcos Lima",
      text: "Loja extremamente séria e confiável. Todo o processo de avaliação e financiamento foi rápido e sem burocracia.",
        ...
        <div className="grid gap-5 lg:grid-cols-4 lg:gap-6">
          <Card className="flex flex-col items-center justify-center border-white/5 bg-[#121212] p-6 text-center rounded-2xl">
            <img
              ...
              alt="Google"
              className="h-6 mb-4 brightness-0 invert opacity-90"
            />
          ...
          <div className="grid gap-4 md:grid-cols-3 lg:col-span-3">
            {reviews.map((r, i) => (
              <Card
                key={i}
                className="relative border-white/5 bg-[#121212] p-5 rounded-2xl flex flex-col justify-between hover:border-white/10 transition-all"
              >
                <Quote className="absolute top-5 right-5 w-6 h-6 text-[#E8231F] opacity-20" />
                <p className="text-xs sm:text-sm text-gray-300 leading-relaxed italic mb-5 relative z-10">
                  "{r.text}"
                </p>
                <div className="flex items-center gap-3">
                  <img
                    src={r.photo}
                    alt={r.name}
                    className="w-10 h-10 rounded-full border-2 border-[#E8231F]/40"
                  />
                  <div>
